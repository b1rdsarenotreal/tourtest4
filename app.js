...
let qualEditing = null; // {round, slot}
...
function handleAddQualifiersToMain(){
  ...
  msg.textContent = added > 0
    ? "Added " + added + " qualifier" + (added===1?"":"s") + " to the main draw's unseeded entrants."
    : "All qualifiers are already in the main draw field.";
  msg.className = "form-msg ok";
  renderBracketUnseededList(t);
}

const QUAL_ROW_H = 108;
function renderQualBracketRounds(t){
  ...
  rounds.forEach((roundObj, r) => {
    ...
    roundObj.matches.forEach((m, i) => {
      ...
      const isEditing = qualEditing && qualEditing.round === m.round && qualEditing.slot === m.slotIndex;
...
function buildQualMatchCard(t, m){
  ...
  if(m.status === "ready"){
    if(qualEditing && qualEditing.round === m.round && qualEditing.slot === m.slotIndex){
      card.appendChild(buildQualEntryForm(t, m));
    } else {
      const btn = el("button", {class:"btn btn-small btn-primary"}, ["Enter Result"]);
      btn.addEventListener("click", () => {
        qualEditing = {round: m.round, slot: m.slotIndex};
        renderQualBracketRounds(t);
      });
...
function buildQualEntryForm(t, m){
  ...
  cancelBtn.addEventListener("click", () => { qualEditing = null; renderQualBracketRounds(t); });
